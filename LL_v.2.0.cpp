...
typedef struct st { //описание структуры стека
  char elem[NMAX]; // массив для хранения элементов стека
...
void treeprint (btree *root) // ïå÷àòü äåðåâà
{
	if (root != NULL)
	{
		treeprint(root->left);
...
void  counting (btree *root, int *dim_n, int *dim_t) // подсчет ко-ва всех терминалов и нетерминалов
{
//	int count_n = 0;
//	int count_t = 0;
		if (root != NULL)
...
int main()
{
	...
	int supCol = 2; //размерность столбцов вспомогательной таблицы парсингаà
